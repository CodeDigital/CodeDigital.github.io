...
function isInside(poly, point){
    ...
    if(cPoints.length > 0 && ccPoints.length > 0){
        ...
        if(cPoints.length >= ccPoints.length){
            ...
            while(ccPoints.includes(fp)){
                fpIndex = (fpIndex == 0 ? poly.length:fpIndex) - 1;
                fp = poly[fpIndex];
             //   concaveSet.unshift(fp);
            }
            ...
            //polyCopy.splice(polyIndex,1);

            // let pv = concaveSet[concaveSet.length - 1]
            // concaveSet.forEach(function(p, i){
            // //     // if(i != 0){
            // //     if(i != 0 && i != concaveSet.length - 1){
            // //         if(concaveSet.length > 3){
            // //             if(i < concaveSet.length - 2){
            // //                 let removeIndex = polyCopy.indexOf(p);
            // //                 polyCopy.splice(removeIndex, 1);
            // //             }
            // //         }else{
            // //             let removeIndex = polyCopy.indexOf(p);
            // //             polyCopy.splice(removeIndex, 1);
            ...
            // removeIndex = polyCopy.indexOf(concaveSet[concaveSet.length - 1]);
            // //polyCopy.splice(removeIndex, 1);

            // let firstVertex = concaveSet[0];

            // fpIndex = (polyIndex == 0 ? poly.length:polyIndex) - 1;
            // let pointsUpTo = 0;
            ...
            for(let i = 0; i < polyCopy.length; i++){
                ...
                if(!concaveSet.includes(v)){
                    ...
                    if(isInside(concaveSet, v)){
                        addSet.push(v);
                        addInside = true;
                        // pointsUpTo = fpIndex - i;
                        // //console.log('is inside: ', v)
                        // //concaveSet.push();
                    }
                }
            }
            ...
            // for(let i = 1; i <= pointsUpTo; i++){
            //     let pushIndex = (fpIndex - i >= 0 ? (fpIndex - i):(polyCopy.length - 1));
            //     concaveSet.unshift(polyCopy[pushIndex]);
            // }

            // let firstConcavePoint = concaveSet[1];
            // for(let i = 1; i < concaveSet.length; i++){
            //     let v = concaveSet[i];
            //     if(ccPoints.includes(v)){
            //         firstConcavePoint = v;
            //         break;
            //     }
            // }
            ...
            // let removeIndex;
            // if(concaveSet.length <= 4){
            //     removeIndex = polyCopy.indexOf(concaveSet[concaveSet.length - 2]);
            // }else{
            //     removeIndex = polyCopy.indexOf(concaveSet[concaveSet.length - 3]);
            // }
            
            // let removeIndex = polyCopy.indexOf(firstConcavePoint);
            ...
            // polyCopy = polyCopy.filter(function(v){
            //     if(concaveSet.includes(v)){
            //         // if(!compareVectors(firstVertex, v) && concaveSet.indexOf(v) < concaveSet.length - 1){
            //         if(concaveSet.indexOf(v) > 0 && concaveSet.indexOf(v) < concaveSet.length - 1){
            //             return false;
            ...
            // pv = polyCopy[polyCopy.length - 1];
            // polyCopy.forEach(function(v, i){
            ...
            //console.log('going inside ', point);
            ...
            // let firstConcavePoint = concaveSet[0];
            // for(let i = 0; i <concaveSet.length; i++){
            //     let v = concaveSet[i];
            //     if(cPoints.includes(v)){
            //         firstConcavePoint = v;
            //         break;
            //     }
            // }
            ...
            // let removeIndex = polyCopy.indexOf(concaveSet[1]);
            // let removeIndex = polyCopy.indexOf(firstConcavePoint);
            //polyCopy.splice(removeIndex, 1);
            let newpCopy = []
